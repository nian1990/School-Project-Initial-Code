...
```{r}
#plug 0 value into attributes missing from test set after one hot encoding (it's different everytime running the code as training & testing are randomly selected)
x_test1$ZIPCODE2446 <- 0
x_test1$U_KITCH_TYPE1F...1.Full.Eat.In.K <- 0
#x_test1$U_HEAT_TYPN...None <- 0
#x_test1$U_INT_FING...Good <- 0
x_test1 <- x_test1 %>% relocate(ZIPCODE2446, .after = ZIPCODE2215)
x_test1 <- x_test1 %>% relocate(U_KITCH_TYPE1F...1.Full.Eat.In.K, .after = U_BTH_STYLE3S...Semi.Modern)
#x_test1 <- x_test1 %>% relocate(U_HEAT_TYPN...None, .after = U_HEAT_TYPF...Forced.Hot.Air)
#x_test1 <- x_test1 %>% relocate(U_INT_FING...Good, .after = U_INT_FINE...Elaborate)
```
...
Results for Scenario 1 (with outliers in training set):

1. Random Forest (both 5 trees and 10 trees) generates the best performance metrics: Highest R Squared (both over 90%) and lowest RMSE and MAE.
   The multilinear regression also generates decent performance metrics on testing data.
   K Nearest Neighbour Regression has the lowest performance. The performance might be increased if we increase the number of k; however, this algorithm takes significantly long time to train the model. Therefore, it's not ideal to use when there are many features and large dataset.

2. All modelings show a bit of overfitting. 
...
x_test2$ZIPCODE2446 <- 0
#x_test2$U_KITCH_TYPE1F...1.Full.Eat.In.K <- 0
#x_test2$U_HEAT_TYPO...Other <- 0
x_test2 <- x_test2 %>% relocate(ZIPCODE2446, .after = ZIPCODE2215)
#x_test2 <- x_test2 %>% relocate(U_KITCH_TYPE1F...1.Full.Eat.In.K, .after = U_BTH_STYLE3S...Semi.Modern)
#x_test2 <- x_test2 %>% relocate(U_HEAT_TYPO...Other, .after = U_HEAT_TYPN...None)
...
Results for Scenario 2 (without outliers in training set):

1. In contrast to Scenario 1, Random Forest (both 5 trees and 10 trees) generates the worst performance metrics on testing set.
   The multilinear regression has the highest performance metrics on testing data, but still lower than Scenario 1.
   

2. All models trained with training dataset without outliers show significant overfitting. 
